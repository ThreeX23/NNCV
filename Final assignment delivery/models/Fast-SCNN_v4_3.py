...
v3: Two skip connections are added to the v2 inside the classifier to increase acc
___ The data is retrieved from the downsample section.

v4: More channels. In GFE() the number of parameters has been tuned down.
...
class Classifier(nn.Module):
    ...
    def __init__(self, in_channels, num_classes, stride):
        ...
        self.dsconv2 = DSConv(in_channels + 64, in_channels, stride)
        self.dsconv3 = DSConv(in_channels + 32, 64, stride)  ######
        self.conv = nn.Sequential(
            nn.Dropout(0.1),
            nn.Conv2d(64, num_classes, kernel_size=1)  ######
        )
        self.db = conv1 = nn.ConvTranspose2d(in_channels, in_channels, kernel_size=4, stride=2,
                                             padding=1)  # doubles height and width
    ...
    def forward(self, x, skip1, skip2, size):
        ...
        x4 = self.conv(x3)
        x5 = F.interpolate(x4, size, mode='bilinear', align_corners=True)
        return x5
...
class Model(nn.Module):
    ...
    def __init__(self, in_channels=3, n_classes=19):
        ...
        self.downsample = LearningToDownsample(in_channels=in_channels, channel_1=32, channel_2=64, out_channels=128)
        self.feature_extractor = GlobalFeatureExtractor(in_channels=128, out_channels=256, t=6)
        
        self.med_fe = MediumFeatureExtractor(in_channels=128, out_channels=192, t=6)
        self.mid_fusion = FeatureFusion(high_res_in_channels=128, low_res_in_channels=192, out_channels=192)
        
        self.fusion = FeatureFusion(high_res_in_channels=192, low_res_in_channels=256, out_channels=256)
        self.classifier = Classifier(in_channels=256, num_classes=n_classes, stride=1)
