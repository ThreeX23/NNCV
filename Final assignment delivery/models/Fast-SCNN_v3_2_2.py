...
Seemingly, accuracy is not high enough, therefore research question becomes:
Can the Fast-SCNN be improved to reach 80% of UNet performance?

In previous versions:
v2: According to [1] the convolutions in the classifier phase boost performance,
___ therefore one DS convolution is added in the classifier.

In this version:
Two skip connections are added to the v2 inside the classifier to increase acc
The data is retrieved from the downsample section.
One more Bottleneck skip connection has been added.
SE block and refinement head.
"""
...
class SEBlock(nn.Module):
    def __init__(self, channels, reduction=16): #16 for big, 8 or 4 for small
...
class LearningToDownsample(nn.Module):
    ...
    def __init__(self, in_channels, channel_1, channel_2, out_channels):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels, channel_1, kernel_size=3, stride=2, padding=1, bias=False),
...
class MediumFeatureExtractor(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, t):
        ...
        self.bottleneck2 = nn.Sequential(
            Bottleneck(64, 96, t, stride=1),
            Bottleneck(96, 96, t, stride=1),
            Bottleneck(96, 96, t, stride=1)

        )
        self.ppm = PPM(96, out_channels)
...
class FeatureFusion(nn.Module):
    ...
    def __init__(self, high_res_in_channels, low_res_in_channels, out_channels):
        ...
        self.se = SEBlock(out_channels)  # <<<<<< HIER zit de SE-block

    def forward(self, high_res_feature, low_res_feature):
        low_res_feature = F.interpolate(low_res_feature, high_res_feature.size()[2:], mode='bilinear',
                                        align_corners=True)
        ...
        out = self.se(out)  # <<<<<< ACTIEVE CHANNEL ATTENTION
...
class Classifier(nn.Module):
    ...
    def __init__(self, in_channels, num_classes, stride):
        ...
        self.dsconv2 = DSConv(in_channels + 48, in_channels, stride)
        self.dsconv3 = DSConv(in_channels + 32, 64, stride)  ######
        self.refinement = RefinementHead(64, num_classes)
        self.db = conv1 = nn.ConvTranspose2d(in_channels, in_channels, kernel_size=4, stride=2, padding=1)  # doubles height and width

    def forward(self, x, skip1, skip2, size):
        x1 = self.dsconv1(x)
        db1 = self.db(x1)
        cat1 = torch.cat([db1, skip2], axis=1)
        x2 = self.dsconv2(cat1)
        db2 = self.db(x2)
        cat2 = torch.cat([db2, skip1], axis=1)
        x3 = self.dsconv3(cat2)  ######
        x5 = self.refinement(x3, target_size=size)
        return x5
...
class Model(nn.Module):
    ...
    def __init__(self, in_channels=3, n_classes=19):
        ...
        self.se_skip1 = SEBlock(32)  # skip1 heeft 32 kanalen
        self.se_skip2 = SEBlock(48)  # skip2 heeft 48 kanalen

        self.feature_extractor = GlobalFeatureExtractor(in_channels=64, out_channels=128, t=6)
        self.med_fe = MediumFeatureExtractor(in_channels=64, out_channels=96, t=6)
        self.mid_fusion = FeatureFusion(high_res_in_channels=64, low_res_in_channels=96, out_channels=96)
        self.fusion = FeatureFusion(high_res_in_channels=96, low_res_in_channels=128, out_channels=128)
        self.classifier = Classifier(in_channels=128, num_classes=n_classes, stride=1)

    def forward(self, x):
        size = x.size()[2:]
        skip1, skip2, low_level_features = self.downsample(x)

        # SE attention op de skip connections
        skip1 = self.se_skip1(skip1)
        skip2 = self.se_skip2(skip2)

        mid_level_features = self.med_fe(low_level_features)
        high_level_features = self.feature_extractor(low_level_features)
        mid_fused = self.mid_fusion(low_level_features, mid_level_features)
        fused_features = self.fusion(mid_fused, high_level_features)
        return self.classifier(fused_features, skip1, skip2, size)
